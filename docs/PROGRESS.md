...
| _(pending)_ | UI design-system hardening pass: `Badge`/`Card`/`Section` primitives, severity/status color semantics, lifecycle-action button emphasis, Geist-vs-Arial font fix, custom `not-found.tsx` — see "UI design-system hardening pass (post–Milestone 2)" above |
