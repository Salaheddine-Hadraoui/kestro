...
**IN PROGRESS: Phase 2 — Search + Case Export milestone.** The Search slice (Tasks 1-4: free-text search on `Case.title` and `Alert.source`/`summary`, both backend and frontend) is **complete and verified** as of this checkpoint (Task 5). Case Export (the remaining half of this milestone) **has not started**.

**Task 5 verification results:**
- Backend (`apps/api/`): `npx jest` 135/135 passing across 10 suites (was 127; +8 for the two new search-filter unit-test files' additions), `npx tsc --noEmit` clean, `npm run test:e2e` 102/102 across 8 suites (was 98; +4 for the `?q=` match + over-length-400 e2e cases on both Cases and Alerts), `npx prisma validate` clean, `npx prisma migrate status` clean (7 migrations, no drift — this slice made no schema change, as expected). **`npm run lint` is NOT clean: 7 pre-existing `@typescript-eslint` errors**, all introduced by Tasks 1-2's own committed test-file changes (`no-base-to-string` on the extended Prisma-mock `contains`-matching helpers in `cases.service.spec.ts`/`alerts.service.spec.ts`/`cases.e2e-spec.ts`/`alerts.e2e-spec.ts`, plus `no-unsafe-member-access` on untyped `response.body.data` in two e2e assertions) — see "Known technical debt" below. Not fixed in this task, which is verification-only per its own scope (`docs/PROGRESS.md` is the only file this task modifies); flagged here as a real gate finding, not a formality.
- Frontend (`apps/web/`): `npx jest` 203/203 passing across 45 suites (was 193/45; +10 for the two pages' new search-input/trim/whitespace/length tests plus their service-layer `q` passthrough tests), `npx tsc --noEmit` clean, `npm run lint` clean, `npm run build` clean, emitting the identical route list as before (`/`, `/_not-found`, `/alerts`, `/alerts/[id]`, `/cases`, `/cases/[id]`, `/cases/new`, `/login`, `/session-expired` plus the Proxy) — confirming Search added no new route, exactly as scoped.
- Live walkthrough against the real NestJS API and a live `next dev` server, both against `kestro-postgres-dev` (confirmed empty before and after): one throwaway Analyst (`sdd-verify-analyst@example.com`) was inserted directly via `psql` (user creation is Lead-gated); through the real API, three throwaway Cases and three throwaway Alerts with distinct, `sdd-verify-`-prefixed titles/sources/summaries were created. Confirmed against real Postgres (not the Jest in-memory fake): `GET /v1/cases?q=vpn` and `?q=VPN` both returned only the matching case, proving case-insensitive `contains`; `GET /v1/alerts?q=<source substring>` and `?q=<summary substring>` each matched on their respective field; combining `q=sdd-verify` with `severity=high` shrank the 3-result baseline to 1 on both Cases and Alerts (and to 0 with `severity=critical`), confirming AND semantics — result count never grew relative to the `q`-only query; a 201-character query 400'd on both endpoints while a 200-character query still 200'd (the DTOs' exact `MaxLength(200)` boundary). Separately, fetching the real rendered `/cases` and `/alerts` pages (via `curl` with the session cookies from a real login, matching prior milestones' `$ACTION_*` Server-Action-protocol technique for the login form itself — the search forms themselves are plain `method="GET"` navigation forms, not Server Actions, so no such protocol was needed for search) confirmed the new Search input renders on both pages, and that `?q=vpn`/`?q=phishing` round-tripped correctly: the input's `value` echoed the URL param back, and the rendered list showed only the matching throwaway row. All throwaway rows (cases, alerts, timeline events, case_alerts, refresh tokens, the one user) were deleted afterward and the dev DB confirmed back to empty (0 rows in every table checked), matching how it was found; both dev servers were then stopped and confirmed down.

## Next planned milestone

The current milestone (Search + Case Export, see above) is the answer to "what's next" as of this writing — its Search slice is done and verified; Case Export has not started. Once the full milestone completes, still-open alternatives from before it was scoped remain available for whatever comes after: a Dashboard, Phase 1's remaining named items (richer metrics — docs/ROADMAP.md), or a second, smaller hardening pass over the still-deferred B-tier findings (see "Known technical debt" below). Playbooks, Knowledge, AI, and Integrations remain explicitly out of scope per CLAUDE.md until a future phase actually scopes them.
...
- **`npm run lint` fails in `apps/api/` with 7 `@typescript-eslint` errors**, introduced by Tasks 1-2 of the Search + Case Export milestone's own test-file changes (`no-base-to-string` on the extended Prisma-mock `contains`-matching helpers added to `cases.service.spec.ts`, `alerts.service.spec.ts`, `test/cases.e2e-spec.ts`, `test/alerts.e2e-spec.ts`; plus `no-unsafe-member-access` on untyped `response.body.data` in two e2e assertions in the latter two files). Test-only, not production code; all other verification (unit/e2e tests, `tsc`, `prisma validate`/`migrate status`) is clean. Surfaced by Task 5's verification checkpoint; not fixed there since that task is scoped to verification and `docs/PROGRESS.md` only — should be fixed before or alongside Case Export.
