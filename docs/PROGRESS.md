...
Adds Hypotheses and Evidence sections to the case detail page, completing the frontend for the Hypothesis ↔ Evidence milestone (see above) which had shipped the backend months earlier with no UI to consume it. Executed via the subagent-driven-development workflow (fresh implementer subagent per task, task-scoped spec+quality review after each), across 6 tasks. **Zero backend changes** — every route this milestone consumes (`POST`/`GET /v1/cases/:caseId/hypotheses`, `POST .../validate`, `POST .../reject`, `POST`/`GET .../hypotheses/:hypothesisId/evidence`, `POST`/`GET /v1/cases/:caseId/evidence`) already existed exactly as built by the Hypothesis ↔ Evidence milestone; nothing in `apps/api/` was touched, and no new database migration was needed.

**What was built** (all under `apps/web/`, on top of the Cases Workspace and its UI hardening pass):
- `lib/api/types.ts` gained `Hypothesis`, `HypothesisStatus`, `Evidence`, and `EvidenceType` types mirroring the existing backend DTOs; `lib/badge-tones.ts` gained `HYPOTHESIS_STATUS_BADGE_TONE` (`proposed` = neutral, `validated` = green, `rejected` = purple — rejecting a hypothesis is a normal investigative outcome, not an alarm, so it deliberately does not get a red/warning tone, mirroring `AlertStatus.dismissed`'s same reasoning).
- Two new typed service modules: `features/investigations/service.ts` (`listHypotheses`, `proposeHypothesis`, `validateHypothesis`, `rejectHypothesis`, `linkEvidenceToHypothesis`) and `features/evidence/service.ts` (`listEvidence`, `addEvidence`), both thin `apiFetch` wrappers matching the exact pattern `features/cases/service.ts` and `features/alerts/service.ts` already established.
- Five new Server Actions on the existing `app/(workspace)/cases/[id]/actions.ts` (`proposeHypothesisAction`, `validateHypothesisAction`, `rejectHypothesisAction`, `linkEvidenceAction`, `addEvidenceAction`), each reusing the pre-existing `CaseActionState` shape (`{ error?: string }`) rather than inventing a new one, with their own matching forms (`propose-hypothesis-form.tsx`, `validate-hypothesis-form.tsx`, `reject-hypothesis-form.tsx`, `link-evidence-form.tsx`, `add-evidence-form.tsx`) built only from the pre-existing `Button`/`TextField`/`FormError` primitives — no new UI primitive was needed.
- The case detail page (`app/(workspace)/cases/[id]/page.tsx`) gained a "Hypotheses" and an "Evidence" `Section`, fetched alongside the existing case/timeline data via one `Promise.all`. Hypotheses render as `Card`s with their status badge, conclusion (once validated), and — while still `proposed` and the case isn't `RESOLVED` — inline Validate/Reject/Link-evidence forms; Evidence renders as `Card`s with type/source/timestamp/content. Both directions of the Hypothesis↔Evidence link are shown: a hypothesis card with linked evidence shows "Linked evidence: {source}", and an evidence card with a `hypothesisId` shows "Linked to hypothesis: {statement}" (resolved via an in-page `Map` built from the fetched hypotheses, no extra request).

**Three UI decisions, confirmed with the project owner and recorded here so a future reader treats them as deliberate rather than oversights**:
1. **The Hypotheses and Evidence sections are always visible** (not collapsed/hidden behind a toggle) — consistent with every other section on the case detail page (Linked alerts, Actions, Notes & Comments), all of which are always-rendered `Section`s.
2. **Evidence type is rendered as a plain-text label** (`{item.type} · {item.source}`), not a `Badge` — unlike Severity/Case-status/Alert-status/Hypothesis-status, evidence type is a descriptive classification with no risk/urgency semantic to color-code, so giving it a badge tone would manufacture a meaning that isn't there.
3. **The reject-hypothesis button uses the `secondary` variant, not `warning`** — rejecting a hypothesis is a routine, expected investigative outcome (ruling something out), not an urgency-raising or flow-overriding action like `escalate`/`accept_escalation`/`reopen`, which are the only actions that get `warning`'s amber treatment elsewhere on this page.

**Verification**: from `apps/api/` — `npx jest` 127/127 (10 suites), `npx tsc --noEmit` clean, `npm run lint` clean, `npx prisma validate` clean, `npx prisma migrate status` clean (7 migrations, no drift), `npm run test:e2e` 98/98 (8 suites) — all identical to the counts before this milestone, confirming the zero-backend-change claim. From `apps/web/` — `npx jest` 193/193 across 45 suites (was 163/38; +30 for the new types/badge-tone, service layers, Server Actions/forms, and case detail page integration tests), `npx tsc --noEmit` clean, `npm run lint` clean, `npm run build` (Next 16.3.0/Turbopack) clean, emitting the same route list as before (`/`, `/_not-found`, `/alerts`, `/alerts/[id]`, `/cases`, `/cases/[id]`, `/cases/new`, `/login`, `/session-expired` plus the Proxy) — confirming no new route or nav item, exactly as scoped.

A live walkthrough was run against the real NestJS API and a live `next dev` server, using `curl` against the actual rendered HTML and real Server Action submissions (matching Next 16's `$ACTION_*` hidden-field progressive-enhancement protocol, since no browser-automation tool was available in this environment): one throwaway Analyst (plus a second throwaway Analyst, not assigned to the case, for the 403 check) was inserted directly via `psql` (user creation is Lead-gated and the dev DB starts empty, the same unavoidable exception already used by prior milestones' walkthroughs), then one case was created through the real API (`POST /v1/cases`, `201`). As that Analyst — two hypotheses were proposed and both confirmed rendering with the neutral `proposed` badge; two evidence items of different types (`LOG`, `URL`) were added and both confirmed rendering with their type/source/timestamp/content; the `LOG` item was linked to the first hypothesis, confirming both link directions rendered ("Linked evidence: vpn-gateway-01 auth.log" on the hypothesis card, "Linked to hypothesis: Attacker gained initial access via a compromised VPN credential" on the evidence card); the first hypothesis was validated with a conclusion statement, confirming its badge turned green, the conclusion rendered, and its Validate/Reject/Link-evidence controls disappeared (terminal state); the second hypothesis was rejected with no conclusion, confirming its badge turned purple and its controls likewise disappeared; the case was then walked through its full remaining lifecycle (`begin_triage` → `start_investigation` → `begin_mitigation` → `begin_verification` → `resolve`, all `303`) to `RESOLVED`, confirming the Propose-hypothesis and Add-evidence forms (and the pre-existing Notes/Comments forms) all disappeared while the previously-recorded hypotheses and evidence remained fully visible; finally, the second throwaway Analyst — who has no access to the case — was confirmed to still get the pre-existing 403 `EmptyState` ("You don't have access to this case"), unaffected by this milestone. All throwaway rows (evidence, hypotheses, timeline events, the case, refresh tokens, both users) were deleted afterward and the dev DB confirmed back to empty (0 users/cases/hypotheses/evidence/timeline_events/alerts/refresh_tokens); both dev servers were then stopped.

Task 3's task-scoped review had been interrupted mid-run at the checkpoint noted above, with its verdict left unrecorded. Its one outstanding finding (the link-evidence picker's option labels showing only `source`, not enough context given there's no unlink path) was surfaced and fixed as part of this milestone's final review (`f6e476d`), which also added the missing hypothesis-to-evidence link-direction test coverage — closing out Task 3's review gap alongside the rest of the final review's findings, rather than through a separately re-run Task 3 verdict.

### Search + Case Export

Closes out Phase 1's last two named-but-unbuilt items (`docs/ROADMAP.md`: "search/filter across cases and alerts, case export (PDF/summary)"), as two independent, sequentially-verified slices — Search implemented and fully verified first, Case Export second, per the approved architecture. Executed via the subagent-driven-development workflow across 8 plan tasks plus one mid-milestone lint ruling and one final-review fix wave, all individually reviewed.

**Search slice** (`apps/api/src/cases/dto/list-cases-query.dto.ts`, `cases.service.ts`; `apps/api/src/alerts/dto/list-alerts-query.dto.ts`, `alerts.service.ts`; `apps/web/features/cases/service.ts`, `apps/web/app/(workspace)/cases/page.tsx`; `apps/web/features/alerts/service.ts`, `apps/web/app/(workspace)/alerts/page.tsx`): an optional `q` query parameter (`@IsOptional() @IsString() @MaxLength(200)`) added to `GET /cases` (case-insensitive `contains` on `Case.title`) and `GET /alerts` (case-insensitive `contains` OR'd across `Alert.source`/`Alert.summary`), added inside the same `Prisma.*WhereInput` object each `findAll` already builds — so the Analyst-visibility clause is never bypassed and search always ANDs with the existing status/severity/assigneeId filters. Zero schema changes; zero new endpoints. Each list page gained a plain `<input type="search">` in its existing filter form, following the exact sanitize-or-drop convention already used for `offset`/`assigneeId`: a whitespace-only or over-200-character query is dropped entirely before it reaches the backend, never truncated.

**Case Export slice** (`apps/web/lib/case-export.ts`, `apps/web/app/(workspace)/cases/[id]/export/route.ts`, `apps/web/app/(workspace)/cases/[id]/page.tsx`): a Next.js Route Handler at `/cases/[id]/export` — the second Route Handler in this codebase, after `app/session-expired/route.ts` — reusing the exact service functions the case detail page already calls (`getCase`, `listCaseTimelineEntries`, `listHypotheses`, `listEvidence`, `listUsers`) to render a case's metadata, resolution, hypotheses (with author/conclusion/linked evidence), evidence (with author/linked hypothesis), and notes/comments (via the existing `extractHumanEntries`, never raw internal `TimelineEventType` values) as a downloadable Markdown file (`Content-Disposition: attachment; filename="case-{id}-{YYYY-MM-DD}.md"`). Zero new backend code, zero new authorization logic — the export inherits `getCase()`'s existing 403/404 behavior entirely. Works for a case in any lifecycle state. A plain `<a>` link (not `next/link`'s `Link`, to avoid triggering the export's 5 backend calls on every case-page-view prefetch) was added to the case detail page's header, deliberately separate from the "Actions" section so it can never read as a lifecycle transition.

**Two rulings made during execution, recorded here for the record:**
1. The plan's own Task 1/2 verification steps ran only `jest`/`tsc`, never `lint` — so 7 `@typescript-eslint` errors introduced by those tasks' own test-file changes (extending four hand-rolled Prisma-fake matchers to understand `{contains, mode}`/`OR`) went undetected until the Search verification checkpoint. Fixed in `490c4ab` (type-narrowing only, zero behavior change, independently re-reviewed) rather than logged as deferred debt, since it was newly introduced by this milestone and every prior milestone here has kept `lint` clean at its gate.
2. The final whole-branch review found 4 Important findings, all fixed in one dispatch (`0994445`) and independently re-reviewed as addressed with no new breakage: a stale "lint fails" claim this same document had briefly carried (see above, now corrected); `<Link>` → plain `<a>` for the export action (prefetch was silently executing the export's backend calls on every case-page view); a `timelineTotal`-driven disclosure line added to the export when a case's notes/comments exceed the existing 100-event timeline window (the case detail page already discloses this same limit — the export previously didn't); and the export route now catches `SessionExpiredError` and redirects to `/session-expired`, matching every other page's behavior instead of surfacing a generic 500.

**Verification**: from `apps/api/` — `npx jest` 135/135 (10 suites), `npx tsc --noEmit` clean, `npm run lint` clean, `npx prisma validate`/`migrate status` clean (7 migrations, zero schema diff across the entire milestone — confirmed via `git diff --stat` against the pre-milestone commit), `npm run test:e2e` 102/102 (8 suites). From `apps/web/` — `npx jest` 218/218 (47 suites, was 193/45 before this milestone; +25 across both slices), `npx tsc --noEmit` clean, `npm run lint` clean, `npm run build` clean; the emitted route table now additionally lists `/cases/[id]/export` as a Route Handler entry (expected — Route Handlers appear in Next's full routing table; this is not a new *page* route and has no nav entry), with every other route unchanged, and `apps/web/lib/nav.ts` confirmed byte-identical to before this milestone.

Two live walkthroughs were run against the real NestJS API and a live `next dev` server, both against `kestro-postgres-dev` (confirmed empty before and after each). The first (Search-only, at the Task 5 checkpoint) confirmed real-Postgres case-insensitive `contains` matching on both `Case.title` and `Alert.source`/`summary`, correct AND semantics with existing filters, the exact 200-character boundary, and the frontend `q` field round-tripping through the URL on both list pages. The second (full-milestone, at Task 8) re-confirmed Search had no regression, and additionally: exported a case with two hypotheses (one validated with a conclusion, one rejected), two evidence items (one linked to the validated hypothesis), and a note — confirming every required section, both link directions, and real author names (not raw ids) appeared in the downloaded Markdown, and that none of `status_change`/`alert_linked`/`evidence_added` appeared anywhere in it; exported a second, empty throwaway case and confirmed the graceful "No hypotheses proposed."/"No evidence recorded."/"No notes or comments." lines rendered instead of broken sections; exported a case still in `INVESTIGATING` (not `RESOLVED`) and confirmed it worked with no status gate; and, as a second throwaway Analyst with no access to the first case, confirmed `GET /cases/{id}/export` returns the same 403 the case detail page itself would, proving no authorization bypass exists in the export path. All throwaway rows across both walkthroughs were deleted afterward and both dev DBs confirmed back to empty; all dev servers stopped.

## Current task

No milestone is currently in progress. **Search + Case Export** (see above) is the most recently completed one. The next milestone has not yet been chosen — see "Next planned milestone" below for the open candidates.

## Next planned milestone

Not yet chosen. Open candidates carried forward from before Search + Case Export was scoped: a Dashboard, Phase 1's remaining named item (richer metrics — still "loose" per `docs/ROADMAP.md`'s own open-decisions list, deliberately not scoped as part of this milestone), or a second, smaller hardening pass over the still-deferred B-tier findings (see "Known technical debt" below, now including several new items from this milestone). Playbooks, Knowledge, AI, and Integrations remain explicitly out of scope per CLAUDE.md until a future phase actually scopes them.
...
- The Notes & Comments section's fixed 100-event timeline window now competes for space with Investigation events (hypothesis/evidence activity also writes `timeline_events` rows) — an investigation-heavy case could crowd real analyst notes out of that view faster than before this milestone. Not fixed here; worth revisiting if it becomes a real problem.
- Evidence's `timestamp` field (entered via a `datetime-local` input) is interpreted in the server's timezone, not the submitting analyst's — it happens to round-trip correctly today because the evidence card also renders it server-side with `toLocaleString()`, but this is fragile if that ever changes. Consistent with this page's pre-existing date handling elsewhere; not a new issue introduced here, just newly relevant.
- The link-evidence picker's option labels (source · type · timestamp, widened during this milestone's final review) still don't show full evidence content, and there is still no unlink endpoint — a wrong link remains uncorrectable for the life of the case even with the widened labels.
- ~~`npm run lint` fails in `apps/api/` with 7 `@typescript-eslint` errors, introduced by Tasks 1-2 of the Search + Case Export milestone's own test-file changes (`no-base-to-string` on the extended Prisma-mock `contains`-matching helpers added to `cases.service.spec.ts`, `alerts.service.spec.ts`, `test/cases.e2e-spec.ts`, `test/alerts.e2e-spec.ts`; plus `no-unsafe-member-access` on untyped `response.body.data` in two e2e assertions in the latter two files)~~ — fixed in `490c4ab` (narrowed types before `String()` in the `contains` matchers and added type assertions on the two `response.body.data` accesses); `npm run lint` is clean in `apps/api/` as of this milestone's completion.
- The "an evidence item can only be linked to a hypothesis while that hypothesis is `proposed`" restriction exists only in the frontend (`LinkEvidenceForm` is only rendered for `proposed` hypotheses); the backend's `InvestigationsService.linkEvidence` has no equivalent guard and would accept a link against a `validated`/`rejected` hypothesis if called directly. This is a deliberate, sensible UI narrowing per the plan, not a bug — noted so a future reader doesn't try to "fix" one side to match the other.
- Search's `q` is passed to Prisma's `contains` without escaping `%`/`_`/`\` — not an injection risk (Prisma parameterizes the query), but `_` matches any single character and `%` matches everything, so e.g. `?q=edr_agent` also matches `edr-agent`/`edrXagent`, and `?q=%` matches every row. Surprising, not unsafe. From the Search + Case Export milestone's final review; not fixed.
- None of the four hand-rolled Prisma-fake matcher functions used by Search's unit/e2e tests (`cases.service.spec.ts`, `alerts.service.spec.ts`, `test/cases.e2e-spec.ts`, `test/alerts.e2e-spec.ts`) actually check Prisma's `mode: 'insensitive'` — all four unconditionally lowercase both sides whenever a `contains` filter is present, so no automated test would catch a regression that dropped case-insensitivity from either service; only the milestone's live walkthroughs verified this against real Postgres. The four matchers are also now near-identical ~18-line copies (they were 1-3 lines each before this milestone) — a shared test helper would stop them drifting. From the Search + Case Export milestone's final review; not fixed.
- Case export (`lib/case-export.ts`) omits the case's linked alerts (shown on the case detail page under "Linked alerts") and `Case.updatedAt`/`Hypothesis.createdAt`/`resolvedAt` — not in this milestone's spec, a scope call rather than a defect, but the triggering alert is arguably the one piece of context a reader of the exported record would expect given `Alert → Case` is the product's central chain. Also, user-authored free text (evidence `content`, note/comment `text`) is interpolated into the Markdown without escaping embedded newlines, so a multi-line value can break out of its list-item indentation. From the Search + Case Export milestone's final review; not fixed.
- The case export response has no `Cache-Control: no-store` header, despite carrying a full case's sensitive content (`docs/CLAUDE.md`'s "treat all case/evidence content as sensitive" applies here too). From the Search + Case Export milestone's final review; not fixed.
- The case `id` is interpolated into the export's `Content-Disposition` filename unescaped — not reachable in practice (`getCase(id)` must already have succeeded, so `id` is a real, well-formed case id by that point), but a pathological id could corrupt or throw on the header rather than degrade gracefully. From the Search + Case Export milestone's final review; not fixed.
...
| `120077b`..`5fa4adc` + this commit | Investigation Workspace: Hypothesis/Evidence types + `HYPOTHESIS_STATUS_BADGE_TONE` (`120077b`), Investigations + Evidence service layers (`623c251`), propose/validate/reject/link-evidence hypothesis actions and forms (`cf0575e`), evidence Server Action and form (`596da58`), Hypotheses and Evidence sections on the case detail page (`5fa4adc`) — zero backend changes; see "Investigation Workspace" above |
| `ae35db4`..`0994445` | Search + Case Export: Cases/Alerts backend search (`ae35db4`, `31bed5e`), Cases/Alerts frontend search (`e7480c2`, `f13d27d`), Search verification checkpoint (`9275a71`) plus a lint-fix ruling (`490c4ab`) and a leftover-formatting commit (`b9c3159`), Case Export helper + Route Handler (`1f5d8f0`), case detail page export link (`3955650`), final-review fix wave (`0994445`) — zero backend changes for Export, zero schema changes for the whole milestone; see "Search + Case Export" above |
